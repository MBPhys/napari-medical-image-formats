...
def image_layer_from_image_sitk(image):
    ...
    if metadata is None:
        image_layer = napari.layers.Image(data, rgb=rgb, scale=scale, translate=translate)
    else: 
        image_layer = napari.layers.Image(data, rgb=rgb, metadata=metadata, scale=scale, translate=translate)  
    return image_layer
...
def napari_get_reader(path):
    ...
    if not path_obj.is_dir():
       return reader_function_path_list
    else:
       return reader_function_dir

def reader_function_dir(path):
    reader = sitk.ImageSeriesReader()
    #if filenames_List:
    #    dicom_names=tuple(folder)
    #else:
    dicom_names = reader.GetGDCMSeriesFileNames(path)
    reader.SetFileNames(dicom_names)
    itkimage = reader.Execute()

    image_layer = image_layer_from_image_sitk(itkimage)
